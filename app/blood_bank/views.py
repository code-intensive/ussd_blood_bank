...
from blood_bank.utils import create_request_from_history
...
def ussd(request: HttpRequest) -> HttpResponse:
    ...
    if text == "":
        ...
        if word_count == NAME_INDEX:
            ...
            request_type = (
                "request" if request_history[REQUEST_TYPE] == "1" else "donation"
            )
            name = request_history[NAME_INDEX].strip().title()
            mobile = request_history[MOBILE_INDEX]
            address = request_history[ADDRESS_INDEX].strip().capitalize()
